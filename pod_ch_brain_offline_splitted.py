...
snapshots_matrix_phi = rbnicsx.backends.FunctionsList(problem.function_space_phi)
snapshots_matrix_mu = rbnicsx.backends.FunctionsList(problem.function_space_mu)
snapshots_matrix_n = rbnicsx.backends.FunctionsList(problem.function_space_n)
...
### DEVIDE THE COSTRUCTION OF THE BASE IN SLOT NOT TO SATURATE MPI COMMUNICATORS ###
id=0
slot=15
if MPI.COMM_WORLD.rank==0:
    file_object = open("report.txt", 'a')
    file_object.write("Starting of the simulation "+str(id)+"\n")
    file_object.close()
for (e_index, e) in enumerate(training_set):
    if (e_index<slot*(id+1) and e_index>=slot*id):
        if MPI.COMM_WORLD.rank==0:
            print(rbnicsx.io.TextLine(str(e_index), fill="#"))
            print("high fidelity solve for epsilon =", e)
        pre=time.time()
        snapshot_phi, snapshot_mu, snapshot_n, times = problem.solve(e, False)
        post=time.time()
        print("Elapsed time for itaration ", e_index, " : ", post-pre)

        if MPI.COMM_WORLD.rank==0:
            print("high fidelity solved")
        snapshot_phi.save(f"snapshots_{serial_or_parallel}", "snapshot_phi_"+ str(e_index))
        snapshot_mu.save(f"snapshots_{serial_or_parallel}", "snapshot_mu_"+ str(e_index))
        snapshot_n.save(f"snapshots_{serial_or_parallel}", "snapshot_n_"+str(e_index))


#### CREDO CHE IL PROBLEMA NELLA PARALLELIZZAZIONE SIA QUESTA FUNZIONE
        eigenvalues_i, modes_i, _ = rbnicsx.backends.proper_orthogonal_decomposition_block([snapshot_phi, snapshot_mu, snapshot_n], [reduced_problem.inner_product_action_phi, reduced_problem.inner_product_action_mu, reduced_problem.inner_product_action_n], N=Nint, tol=0.0)

### NOT WORKING

        for (eigs, mode) in zip(eigenvalues_i, modes_i):
            for (eig, mod) in zip(eigs, mode):
                with mod.vector.localForm() as mod_local:
                    if eig > 0:
                        mod_local[:] *= np.sqrt(eig)
                    else:  # slighlty smaller than zero
                        mod_local[:] = np.zeros_like(mod_local)

    #input("Post eig")
        eigenvalues_phi_i, eigenvalues_mu_i, eigenvalues_n_i = eigenvalues_i
       # print("Eig phi"eigenvalues_phi_i)
       # print(eigenvalues_mu_i)
       # print(eigenvalues_n_i)
        modes_phi_i, modes_mu_i, modes_n_i = modes_i
        #print("Tipo mode :",modes_phi_i[0].getType())
        modes_phi_i.save(f"modes_{serial_or_parallel}/modes_"+str(id), "modes_phi")
        modes_mu_i.save(f"modes_{serial_or_parallel}/modes_"+str(id), "modes_mu")
        modes_n_i.save(f"modes_{serial_or_parallel}/modes_"+str(id), "modes_n")

        fig = go.Figure()
        fig.add_scatter(x=np.arange(eigenvalues_phi_i.shape[0]), y=eigenvalues_phi_i / eigenvalues_phi_i[0], mode="markers+lines")
        fig.update_layout(title="Normalized singular values for phi")
        fig.update_xaxes(title_text="N"),fig.update_yaxes(title_text=r"$\frac{\sigma_N}{\sigma_0}$", type="log", exponentformat="power")
        fig.write_image(f"snapshots_{serial_or_parallel}/singulare_values_phi_"+str(e_index)+".png")
...
if MPI.COMM_WORLD.rank==0:
    file_object = open("report.txt", 'a')
    file_object.write("End of the simulation "+str(id)+"\n")
    file_object.close()
